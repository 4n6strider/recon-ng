...
class Module(module.Module):
    def __init__(self, params):
         module.Module.__init__(self, params, query='SELECT DISTINCT contact FROM contacts WHERE contact IS NOT NULL ORDER BY contact')
        ...
        self.info = {
            'Name': 'Facebook Query Agent',
            'Author': 'Tom Jones(The Anoduck)',
            'Version': 'pre-v0.0.1-alpha',
            'Description': 'Queries facebook for relevant information concerning a previously discovered name',
            'Comments': [
                'Note: Names need to be placed in a "(Firstname)_(space)_(Lastname)" format in order to search.',
                ],
            }
#### >> Below is garbage left over from the module template###
    # optional method
  #  def module_pre(self):
     ...
     self.br = self.browser()
        ...
        if self.login(username, password):
            for contact_id in contacts:
                self.heading(contact_id, level=0)
                contact_id = self.get_contact_id(len(fname, mname, lname))
                if contact_id:
                    self.get_contacts(str(contact_id))
